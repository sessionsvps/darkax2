...
async def logUserBot():
    load_dotenv()
    api_id = int(28123665)
    api_hash = "8d959e576c2a7d1db5f8a625cb2deb7c"
    phone_number = "51981636041"
    ...
    await client.send_message("@spam18995345darkax12", f'<b>Bot encendido</b>', parse_mode="HTML")
    spammer_group = int("-4566194406")
    ...
    while True:
        ...
        try:
            await client.send_message("@spam18995345darkax12", f"<b>CANTIDAD DE MENSAJES CONSEGUIDOS PARA PUBLICAR</b> <code>{len(messages_list)-1}</code>",parse_mode="HTML")
        except:
            pass
                ...
                if i['group_name'] not in ["Referencias GaelFord","SPAM 2024","Spam 2024","ENVIADOS SPAM","RESPALDO🇵🇪BINS PERU","➳𝒀𝑨𝑷𝑬 𝑫𝑬 𝑬𝑺𝑻𝑨𝑭𝑨𝑫𝑶𝑹𝑬𝑺 ✧","QUEMANDO ESTAFADORES","𝐏𝐄𝐑Ú 𝐀𝐘𝐔𝐃𝐀","Referencias Elmer 💸","🎭 CANAL MUNDO STREAMING PERÚ 🇵🇪","TU MARKETPLACE"]:
                    ...
                    for message_spam in messages_list:
                        ...
                        try:
                            ...
                            await client.send_message("@spam18995345darkax12", f'<b>Error enviando mensajes a {i["group_id"]}</b> - <code>{i["group_name"]}<code>\nCausa:{error}',parse_mode="HTML")
                            resultado = False
                        if resultado:
